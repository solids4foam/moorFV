...
namespace fv
{
    defineTypeNameAndDebug(beamActuatorLine, 0);
...
void beamActuatorLine::calculateS()
{
    if (Pstream::master())
    {
...
scalar beamActuatorLine::eta(const scalar r) const
{
...
beamActuatorLine::beamActuatorLine
(
    const word& name,
...
bool beamActuatorLine::read(const dictionary& dict)
{
    if (option::read(dict))
    {
