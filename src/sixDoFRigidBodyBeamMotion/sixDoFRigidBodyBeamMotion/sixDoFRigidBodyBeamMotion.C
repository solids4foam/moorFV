...
void Foam::sixDoFRigidBodyBeamMotion::applyRestraints()
{
...
Foam::sixDoFRigidBodyBeamMotion::sixDoFRigidBodyBeamMotion(const Time& time)
:
    time_(time),
    motionState_(),
    motionState0_(),
...
Foam::sixDoFRigidBodyBeamMotion::sixDoFRigidBodyBeamMotion
(
...
:
    ...
    solver_(sixDoFFvBeamSolver::New(dict.subDict("solver"), *this))
{
...
Foam::sixDoFRigidBodyBeamMotion::sixDoFRigidBodyBeamMotion
(
    const sixDoFRigidBodyBeamMotion& sDoFRBM
)
:
    time_(sDoFRBM.time_),
...
void Foam::sixDoFRigidBodyBeamMotion::addRestraints
(
    const dictionary& dict,
    const Time& time
)
    ...
    if (dict.found("restraints"))
    {
        ...
        for (const entry& dEntry : restraintDict)
        {
            if (dEntry.isDict())
            {
                ...
                (
                    i++,
                    sixDoFRigidBodyBeamMotionRestraint::New
                    (
                        dEntry.keyword(),
                        dEntry.dict(),
                        time
...
void Foam::sixDoFRigidBodyBeamMotion::addConstraints
(
    const dictionary& dict
)
{
    if (dict.found("constraints"))
    {
        ...
        for (const entry& dEntry : constraintDict)
        {
            if (dEntry.isDict())
            {
                ...
                (
                    i,
                    sixDoFRigidBodyBeamMotionConstraint::New
                    (
                        dEntry.keyword(),
                        dEntry.dict(),
                        *this
...
void Foam::sixDoFRigidBodyBeamMotion::updateAcceleration
(
...
void Foam::sixDoFRigidBodyBeamMotion::update
(
    bool firstIter,
    const vector& fGlobal,
    const vector& tauGlobal,
...
void Foam::sixDoFRigidBodyBeamMotion::status() const
{
...
Foam::tmp<Foam::pointField> Foam::sixDoFRigidBodyBeamMotion::transform
(
...
void Foam::sixDoFRigidBodyBeamMotion::updateXYScale
(
    const pointField& initialPoints,
    const scalar& xDist,
    const scalar& yDist,
