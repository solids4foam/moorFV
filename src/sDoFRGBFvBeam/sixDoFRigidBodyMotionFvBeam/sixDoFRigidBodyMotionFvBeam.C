...
Foam::sixDoFRigidBodyMotionFvBeam::sixDoFRigidBodyMotionFvBeam(const Time& time)
:
    time_(time),
    motionState_(),
    motionState0_(),
...
Foam::sixDoFRigidBodyMotionFvBeam::sixDoFRigidBodyMotionFvBeam
(
...
Foam::sixDoFRigidBodyMotionFvBeam::sixDoFRigidBodyMotionFvBeam
(
    const sixDoFRigidBodyMotionFvBeam& sDoFRBM
)
:
    time_(sDoFRBM.time_),
...
void Foam::sixDoFRigidBodyMotionFvBeam::addRestraints
(
    const dictionary& dict,
    const Time& time
)
    ...
    if (dict.found("restraints"))
    {
        ...
        for (const entry& dEntry : restraintDict)
        {
            if (dEntry.isDict())
            {
                ...
                (
                    i++,
                    sixDoFRigidBodyMotionFvBeamRestraint::New
                    (
                        dEntry.keyword(),
                        dEntry.dict(),
                        time
    ...
    if (dict.found("constraints"))
    {
        ...
        for (const entry& dEntry : constraintDict)
        {
            if (dEntry.isDict())
            {
                ...
                (
                    i,
                    sixDoFRigidBodyMotionFvBeamConstraint::New
                    (
                        dEntry.keyword(),
                        dEntry.dict(),
                        *this
...
void Foam::sixDoFRigidBodyMotionFvBeam::updateAcceleration
(
...
void Foam::sixDoFRigidBodyMotionFvBeam::update
(
    bool firstIter,
    const vector& fGlobal,
    const vector& tauGlobal,
...
void Foam::sixDoFRigidBodyMotionFvBeam::status() const
{
