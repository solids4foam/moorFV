...
)
{
    if (mesh().cellZones().size() > 1)
    {
	    ...
	    // Info << "lineContacts size " << curLineContacts.size() << endl;
...
)
{
    ...
    forAll(contact().pointContacts(), pcI)
    {
	    ...
	    // SB: Should contact Force derivative[1] be negative of the owner?
	    ...
	    // Derivation available in point contact wriggers 1997 paper	     
	    ...
	    // SB: 20/05/2022
	    // Commented out because adding this to implicit part
	    // is increasing the total iterations
	    tensor dFcDzeta =  tensor::zero;
	//	0.5*
	//	(
	//	    - (dRdZeta*invM[0][1]*delta)
	//	    + (neiDRdZeta*invM[1][1]*delta)
	//	);
	    
	    //   Info << "dFcDzeta " << dFcDzeta << endl;
	     
	    dFcDzeta = mag(contactForce[sI])
		*(ImNN & dFcDzeta)/contactDistance[sI];
			    
	    
	    // Commented out because adding this to implicit part
	    // is increasing the total iterations
	    tensor dFcDneiZeta =  tensor::zero;
	//	0.5*
	//	(
	//	    - (dRdZeta*invM[0][0]*delta)
	//	    + (neiDRdZeta*invM[1][0]*delta)
	//	);
		
	    dFcDneiZeta = mag(contactForce[sI])
               *(ImNN & dFcDneiZeta)/contactDistance[sI];
	...
	// This new addition dFcDzeta is not included in the moment part
	// DMCoeff is made zero because adding this is also increasing
	// the total iterations
	    ...
	    // SB: 10/06/22 (not being used since dFcDzeta is zero)
	    scalar a = -1;
	    scalar b = 1;
	    // -- //
	    ...
	    scalar neiA = -1; // not used because dFcDneiZeta is zero
	    scalar neiB = 1;
	    
	    if (globalNeiSegI[sI] != globalLowerNeiSegI[sI])
            {		
		neiA = 1;
		neiB = -1;
            }
