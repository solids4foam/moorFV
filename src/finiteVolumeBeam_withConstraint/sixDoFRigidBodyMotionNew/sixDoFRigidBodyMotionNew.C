...
void Foam::sixDoFRigidBodyMotionNew::applyRestraints()
{
...
void Foam::sixDoFRigidBodyMotionNew::applyConstraints(scalar deltaT)
{
    ...
    if (Pstream::master())
    {
        ...
        if (iteration >= maxConstraintIterations_)
        {
            FatalErrorIn
            (
                "Foam::sixDoFRigidBodyMotionNew::applyConstraints"
                "(scalar deltaT)"
            )
                << nl << "Maximum number of sixDoFRigidBodyMotionNew constraint "
                << "iterations ("
        ...
        Info<< "sixDoFRigidBodyMotionNew constraints converged in "
...
Foam::sixDoFRigidBodyMotionNew::sixDoFRigidBodyMotionNew()
:
    motionState_(),
    restraints_(),
    restraintNames_(),
...
{
    addRestraints(dict, time);

    addConstraints(dict, time);
}


Foam::sixDoFRigidBodyMotionNew::sixDoFRigidBodyMotionNew
(
    const sixDoFRigidBodyMotionNew& sDoFRBM
)
:
...
void Foam::sixDoFRigidBodyMotionNew::addRestraints
(
    const dictionary& dict,
    const Time& time
)
    ...
    if (dict.found("restraints"))
    {
        ...
        forAllConstIter(IDLList<entry>, restraintDict, iter)
        {
            if (iter().isDict())
            {
                ...
                (
                    i,
                    sixDoFRigidBodyMotionRestraintNew::New(iter().dict(), time)
                );
...
void Foam::sixDoFRigidBodyMotionNew::addConstraints
(
    const dictionary& dict,
    const Time& time
)
    ...
    if (dict.found("constraints"))
    {
        ...
        forAllConstIter(IDLList<entry>, constraintDict, iter)
        {
            if (iter().isDict())
            {
                ...
                (
                    i,
                    sixDoFRigidBodyMotionConstraintNew::New(iter().dict(),time)
                );
...
void Foam::sixDoFRigidBodyMotionNew::updatePosition
(
    scalar deltaT
)
{
...
void Foam::sixDoFRigidBodyMotionNew::updateForce
(
...
Foam::point Foam::sixDoFRigidBodyMotionNew::predictedPosition
(
...
Foam::vector Foam::sixDoFRigidBodyMotionNew::predictedOrientation
(
...
void Foam::sixDoFRigidBodyMotionNew::status() const
{
